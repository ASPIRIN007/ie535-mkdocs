...
!!! note "Example box"
    - $(0,2,3,0)\succ(0,2,1,4)$ (first difference is at index 3: $3>1$).  
    - $(0,4,5,0)\prec(1,2,1,2)$ (first difference is at index 1: $0<1$).
...
!!! note "Key box — Uniqueness point used in the book"
    Under the assumption that rows of $A$ are linearly independent, this rule yields a **unique** leaving row: if two normalized rows were identical, it would force proportionality in rows of $B^{-1}A$ and contradict $\mathrm{rank}(A)=m$.
...
!!! info "Theorem box — Reduced cost = below/above dual plane (minimization)"
    ...
    - $(A_j,c_j)$ lies **below** the current dual plane  
      $\Longleftrightarrow\ \bar c_j<0$,
    - $(A_j,c_j)$ lies **above** the dual plane  
      $\Longleftrightarrow\ \bar c_j>0$.
